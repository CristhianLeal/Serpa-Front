...
import FilaPorteroEdificio from '../../Components/FilaPorteroEdificio/FilaPorteroEdificio'
...
const UsuariosEdificio = () => {
  ...
  const borrarEdificio = async () => {
      await axios.delete(`https://serpaadministrador.com.ar:8001/edificio/delete-edificio`, {
          data: {
              name: edificioName
          }
      })
  ...
  useEffect(() => {
    const response = axios.get(`https://serpaadministrador.com.ar:8001/edificio/get-edificio`)
      .then((response) => {
  ...
  useEffect(() => {
    ...
    setSelectedEdificio(selectedEdificio);
  }, [edificios, edificioName]);

  useEffect(() => {
    const response = axios.get(`https://serpaadministrador.com.ar:8001/users/obtener-users`)
      .then((response) => {
          ...
          if (result.isConfirmed) {
            ...
            try {
              const response = await axios.post(
                  ...
                  await axios.patch(`https://serpaadministrador.com.ar:8001/edificio/updateEdif/${selectedEdificio._id}`,{
            ...
            <div className="pb-5 w-100">                            
              ...
              <div className='tabla-usuarios fs-6'>
                <div className="table-responsive">
                  <h3 className='text-center mt-4'>INQUILINOS</h3>
                  <table className="table align-middle">
                    ...
                    <tbody className='text-center'>
                      {
                        usuariosPrint
                        .filter(usuario => usuario.tipo !== 'Portero')
                        .sort((a, b) => a.piso - b.piso)
                        .map((usuario) => (<FilaUsuariosEdificio key={usuario._id} usuario={usuario} />))
                      }
                    </tbody>
                  </table>
                  <h3 className='text-center mt-4'>PORTEROS</h3>
                  <table className="table align-middle">
                    <thead className='align-middle'>
                        <tr className='text-center'>
                            <th className="fs-6">Portero</th>
                            <th className="fs-6">Correo</th>
                            <th className="fs-6">Tipo</th>
                            <th className="fs-6">Subir archivo</th>
                            <th className="fs-6">Ult. Recibo de sueldo</th>
                            <th className="fs-6">Perfil</th>
                        </tr>
                    </thead>
                    <tbody className='text-center'>
                      {
                        ...
                        .filter(usuario => usuario.tipo === 'Portero')
                        .sort((a, b) => a.piso - b.piso)
                        .map((usuario) => (<FilaPorteroEdificio key={usuario._id} usuario={usuario} />))
                      }
                    </tbody>
                  </table>
                </div>
